...
class ComplaintsRepository {
  ...
  Future<Either<Failure, ComplaintModel>> raiseComplaint({
    ...
    try {
      ...
      final idempotencyKey = IdempotencyKeyHelper.generate(
        prefix: 'customer-complaint-raise',
        userId: userId ?? '',
        requestId: complaint.bookingId,
      );

      final createResult = await _remoteDataSource.createComplaint(
        complaintData: complaint.toJson(),
        idempotencyKey: idempotencyKey,
      );

      return createResult.fold((failure) => Left(failure), (data) async {
        if (data == null) {
        ...
        try {
          ...
          if (photos.isNotEmpty) {
            ...
            return uploadResult.fold(
              (f) => Right(
                complaint,
              ), // Return created complaint even if photo upload fails
              (_) async {
  ...
  Future<Either<Failure, void>> uploadPhotos({
    ...
    for (int i = 0; i < photos.length; i++) {
      ...
      final idempotencyKey = IdempotencyKeyHelper.generate(
        prefix: 'complaint-photo-presign',
      ...
      final presignResult = await _remoteDataSource.presignComplaintPhoto(
        ...
        data: {'photoIndex': i, 'contentType': contentType},
        idempotencyKey: idempotencyKey,
      );
