...
class MapsCubit extends Cubit<MapsState> {
  ...
  Future<void> determinePosition() async {
    ...
    if (!serviceEnabled) {
      emit(state.copyWith(isLoading: false, errorMessage: 'Location services are disabled.'));
      return;
    }
    ...
    if (permission == LocationPermission.denied) {
      ...
      if (permission == LocationPermission.denied) {
        emit(state.copyWith(isLoading: false, errorMessage: 'Location permissions are denied.'));
        return;
      }
    }

    if (permission == LocationPermission.deniedForever) {
      emit(state.copyWith(isLoading: false, errorMessage: 'Location permissions are permanently denied.'));
      return;
    }
  ...
  Future<void> getAddressFromLatLng(LatLng latLng) async {
    final result = await mapsRepository.getAddressFromLatLng(latLng.latitude, latLng.longitude);
    
    result.fold(
      (failure) => debugPrint(failure.message),
      (address) => emit(state.copyWith(selectedAddressName: address.formattedAddress)),
    );
  }
  ...
  Future<void> getPlaceDetails(String placeId, {required Function(LatLng) onLocationFound}) async {
